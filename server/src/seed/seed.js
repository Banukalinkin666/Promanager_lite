...
async function run() {
  ...
  const owner = await User.create({
    ...
    passwordHash: await User.hashPassword('owner123'),
    role: 'OWNER',
  });
  ...
  const tenant = await User.create({
    ...
    passwordHash: await User.hashPassword('tenant123'),
    role: 'TENANT',
    firstName: 'John',
    lastName: 'Tenant',
    nic: '1234567890123',
  ...
  console.log('Seeded:', { admin: admin.email, owner: owner.email, tenant: tenant.email, property: prop.title, payments: samplePayments.length });
  process.exit(0);
}
